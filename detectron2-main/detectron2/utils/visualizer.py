...
class Visualizer:
    ...
    def draw_dataset_dict(self, dic):
        ...
        if annos:
            ...
            if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
                # colors = [
                #     self._jitter([int(x) / 255 for x in self.metadata.thing_colors[c-1]])
                #     for c in category_ids
                # ]
                colors = [
                    ([int(x) / 255 for x in self.metadata.thing_colors[c-1]])
                    for c in category_ids
                ]
            ...
            # colors = [colors[c-1] for c in category_ids]
